...
for (let i = 0; i < AllGridCells.length; i++) {
    AllGridCells[i].addEventListener("blur", function cellHelper(e) {
        ...
        let { rid, cid } = getAddress(address);
        let cellObject = db[rid][cid];
        console.log(address, rid, content)
...
formulaInput.addEventListener("keydown", function (e) {
    if (e.key == "Enter" && formulaInput.value != "") {
        ...
        let { rid, cid } = getAddress(addressOfTheCell);
        let cellObject = db[rid][cid];
...
function evaluateFormula(formula) {
    ...
    for (let i = 0; i < formulaEntities.length; i++) {
        ...
        if (ascii >= 65 && ascii <= 90) {
            // address -> rid cId
            let cellrcObj = getAddress(formulaEntities[i]);
            // db -> value
...
function setUI(value, rid, cid) {
    ...
    for (let i = 0; i < childrenArr.length; i++) {
        let chriciobj = getAddress(childrenArr[i]);
...
function setFormula(address, formula) {
    ...
    for (let i = 0; i < formulaEntities.length; i++) {
        ...
        if (ascii >= 65 && ascii <= 90) {
            // address -> rid cId
            let parentrcObj = getAddress(formulaEntities[i]);
            // db -> value
