let topLeftCol=document.querySelector('.top_left_cell');
...
// adding event listener to all the the grid cells
...
for (let i=0; i<AllGridCells.length; i++){
    AllGridCells[i].addEventListener('click', function (e){
        // previous cell
        previousCell= addressInput.value;
        if (previousCell !=''){
            getRidCidOfPreviousCell= getAddress(previousCell);
            let prevCell = document
                .querySelector
                (`.grid .cell[rId='${getRidCidOfPreviousCell.rid}'][cId='${getRidCidOfPreviousCell.cid}']`);
        ...
        cCell.style.border = "2px solid #1B9CFC"; // part 1 done
...
// function to get row id and col id 
function getAddress(address){
    let asciival=( address.charCodeAt(0))
    let cid= asciival-65;
